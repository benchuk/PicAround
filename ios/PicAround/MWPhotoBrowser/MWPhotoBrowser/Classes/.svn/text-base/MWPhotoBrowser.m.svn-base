...
//dispath Actions
-(void) dispatchActuallyShareBySMS:(id<MWPhoto>)photo;
-(void) dispatchActuallyPostToFacebook:(id<MWPhoto>)photo;
- (void)dispatchActuallyEmailPhoto:(id<MWPhoto>)photo;
-(void) dispatchActuallyTwittePhoto:(id<MWPhoto>)photo;
@end
    ...
    if (_actionsSheet) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:@"Share" delegate:self
                                                        ...
                                                        otherButtonTitles:NSLocalizedString(@"Save", nil),
                                                            ...
                                                            NSLocalizedString(@"Twitter", nil),nil] autorelease];
            } else {
...
- (void)emailPhoto {
    ...
    if ([photo underlyingImage]) {
        ...
        [self performSelector:@selector(dispatchActuallyEmailPhoto:) withObject:photo afterDelay:0];
    }
}
...
-(void) shareToTwitter
{
    ...
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Preparing", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
        [self performSelector:@selector(dispatchActuallyTwittePhoto:) withObject:photo afterDelay:0];
    }
}
...
-(void) shareToSMS
{
    ...
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Preparing", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
        [self performSelector:@selector(dispatchActuallyShareBySMS:) withObject:photo afterDelay:0];
    }
}
...
-(void) postToFacebook
{
    ...
    if ([photo underlyingImage]) {
        [self showProgressHUDWithMessage:[NSString stringWithFormat:@"%@\u2026" , NSLocalizedString(@"Preparing", @"Displayed with ellipsis as 'Preparing...' when an item is in the process of being prepared")]];
        [self performSelector:@selector(dispatchActuallyPostToFacebook:) withObject:photo afterDelay:0];
    }
}

-(void) dispatchActuallyShareBySMS:(id<MWPhoto>)photo
{
    [self showProgressHUDCompleteMessage:NSLocalizedString(@"Loading...", @"Informing the user an item has finished copying")];
    [self hideControlsAfterDelay]; // Continue as normal...
    if ([_delegate respondsToSelector:@selector(actuallyShareBySMS:)]) {
        [self hideControlsAfterDelay]; // Continue as normal...
        [_delegate actuallyShareBySMS:photo];
    }
...
-(void) dispatchActuallyPostToFacebook:(id<MWPhoto>)photo
{
    ...
    if ([_delegate respondsToSelector:@selector(actuallyPostToFacebook:)]) {
        [self hideControlsAfterDelay]; // Continue as normal...
        [_delegate actuallyPostToFacebook:photo];
    }
...
- (void)dispatchActuallyEmailPhoto:(id<MWPhoto>)photo
{
     [self showProgressHUDCompleteMessage:NSLocalizedString(@"Loading...", @"Informing the user an item has finished copying")];
      [self hideControlsAfterDelay]; // Continue as normal...
    if ([_delegate respondsToSelector:@selector(actuallyEmailPhoto:)]) {
         [self hideControlsAfterDelay]; // Continue as normal...
        [_delegate actuallyEmailPhoto:photo];
    }
}
-(void) dispatchActuallyTwittePhoto:(id<MWPhoto>)photo
{
    ...
    if ([_delegate respondsToSelector:@selector(actuallyTwittePhoto:)]) {
         [self hideControlsAfterDelay]; // Continue as normal...
        [_delegate actuallyTwittePhoto:photo];
    }
}
