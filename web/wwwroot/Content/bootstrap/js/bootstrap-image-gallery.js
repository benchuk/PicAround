...
(function (factory) {
    ...
    $.extend($.fn.modal.Constructor.prototype, {
        ...
        loadImage: function () {
            ...
            var page = url.match(/\/uploadedimages\/(.*)/)[1];
            var eventID = $("#eventID").attr("title");
            var linnnk = "http://picaround.azurewebsites.net/Gallery/GalleryOpenImageApi?eventId=" + eventID + "&imageurl=" + page;
            $('.well1').html('<div  class="fb-comments" data-href="' + linnnk + '" data-num-posts="20" data-width="300"></div>');
            FB.XFBML.parse(document.getElementById('well1'));
            $('.well1').prepend('<p><div class="fb-like" data-href="' + linnnk + '" data-send="true" data-width="300" data-show-faces="true" style="margin-top: 15px;"></div></p>');
            FB.XFBML.parse(document.getElementById('like'));

            var userAppid = this.$links[index].userInfo;
            $.ajax({
                url: 'https://graph.facebook.com/' + this.$links[index].userInfo,
                type: 'GET',
                //data: data,
                dataType: 'json',
                error: function (xhr, status) {
                    ...
                    var userNameUrl = "http://www.picaround.com/gpproxy/GetGooglePlusUserName?userid=" + userAppid;
                    //alert(userNameUrl);
                    $.ajax({
                        url: userNameUrl,
                        type: 'GET',
                        ...
                        error: function (xhr, status) {
                            //alert(xhr.status);
                            var userNameUrl = "http://picaround.com/gpproxy/GetGooglePlusUserName?userid=" + userAppid;
                            //alert(userNameUrl);
                            $.ajax({
                                url: userNameUrl,
                                type: 'GET',
                ...
                success: function (data) {
                    var imgSrc = 'http://graph.facebook.com/' + data.id;
                    var facePage = "https://www.facebook.com/" + +data.id;
                    var imgLink = '<img border=0 src=' + imgSrc + '/picture?type=small />';
                    modal.find('.modal-userInfo').html("<a href=" + facePage + ">" + imgLink + " " + data.name + "</a>");
        ...
        showImage: function (img) {
            ...
            modalImage.addClass("SingleImageGallery");
            var H = parseInt(modalImage.css('max-height').replace('px', ''));
            var W = 0;
            if (H > img.height) {
                H = img.height;
                W = img.width;
